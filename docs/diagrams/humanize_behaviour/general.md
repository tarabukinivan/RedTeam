...
flowchart TD
 subgraph validator["validator"]
        ...
        vl_create_comparor["27.Create comparor"]
 ...
 subgraph cfg_manager["cfg_manager"]
        run_cfg_comparison["32.Run CFG comparison"]
  end
 subgraph bot_container["bot_container"]
        ...
        bot_run_sciprt["25.Run bot script"]
        bot_check_tasks_done["26.Check tasks done"]
  end
 subgraph challenger_container["challenger_container"]
        ...
        ch_checking_score["20.Keep checking score"]
        ch_create_cfg_manager["31.Create CFG manager"]
        cfg_manager
        bot_container
  end
 subgraph miner_container["miner_container"]
        start_miner_container(("Start"))
        mi_server["Running API server"]
        mi_solve_endpoint["[POST] /solve"]
        mi_read_bot_files["16.Read bot files"]
  end
 subgraph humanize_challenge["humanize_challenge"]
        challenger_container
        miner_container
  end
    start(["Start"]) --> validator
    vl_forward --> vl_init_challenge & vl_get_miners_revealed_commits & vl_use_global_scoring & vl_store_miner_results
    ...
    vl_local_scoring --> start_challenge & vl_create_comparor & start_comparison & vl_update_miner_scores
    vl_create_comparor -.-> comparer
    start_comparison --> ct_setup_challenge & cm_compare_outputs
    start_challenge --> ct_setup_challenge & ct_get_ch_tasks
    ct_setup_challenge --> ct_build_challenger & ct_run_challenger
    ...
    ct_download_miners -. Download .-> miner_container
    ct_run_miners <== Run =======> miner_container
    ...
    ch_score_endpoint --> ch_build_bot_image & ch_run_bot_container
    ch_score_endpoint <--> ch_checking_score
    ch_checking_score --> ch_checking_score
    ...
    ch_eval_endpoint == Update score ==> ch_checking_score
    ch_compare_endpoint --> ch_create_cfg_manager & run_cfg_comparison
    ch_create_cfg_manager -.-> cfg_manager
    start_bot_container --> bt_run_main
    bt_run_main --> bot_run_webui_driver
    bot_run_webui_driver --> bot_setup_driver & bot_run_sciprt & bot_check_tasks_done
    ...
    ch_checking_score@{ shape: notch-pent}
